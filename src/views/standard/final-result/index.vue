<template>
	<div class="evaluation-page">
		...
		<div class="actions">
			...
			<el-button type="success" class="action-button" @click="saveResult" :loading="saving">
				保存结果
			</el-button>
		</div>
	</div>
...
// Updated assessmentResultDetail with proper initialization
const assessmentResultDetail = ref({
  ...
  updatedAt: null
	...
	// Updated loadProjectAE to use the data from calculateAssessmentDetailResult


	// Updated loadAssessmentResultDetail to populate the assessmentResultDetail ref
	const loadAssessmentResultDetail = async () => {
	  try {
	    ...
	    // Update the assessmentResultDetail with the results from the backend
	...
	const cards = computed(() => [{
			title: "DFP<br>调整后功能点数",
			value: projectDFP.value || '-',
			unit: "FP"
		},
		{
			title: "SDC<br>调整前的总造价",
			value: formatCurrency(calculationResult.value.projectSDC),
			unit: "元"
		},
		{
			title: "ESDC<br>调整后的总造价",
			value: formatCurrency(calculationResult.value.projectESDC),
			unit: "元"
		},
		{
			title: "AE<br>工作量",
			value: projectAE.value || '-',
			unit: ""
		},
		{
			title: "更新时间",
			value: formatDate(calculationResult.value.updatedAt),
			unit: ""
		}
	]);
  ...
  try {
	...
	onMounted(async () => {
	  await Promise.all([
	    ...
	    loadStandardDetails(),
	    loadAssessmentResultDetail() // This will also load AE since it's part of the same API call
